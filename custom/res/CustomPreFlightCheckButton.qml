...
//    property int id: 0
    ...
    onClicked: {
        ...
        if (manualText !== "" && _manualState !== _statePassed) {
            ...
            _customFunction.setCheckStateTrue(checkId)
        } else if (_manualState == _statePassed && !_vehicleFlying) { //new
            _manualState = _statePending //uncheck
            _customFunction.setCheckStateFalse(checkId)
        }
    }
