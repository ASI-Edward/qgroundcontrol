...
//    property variant usernameList
    ...
    Component.onCompleted: {
        //new
        QGroundControl.settingsManager.appSettings.useChecklist.rawValue = true
        QGroundControl.settingsManager.appSettings.enforceChecklist.rawValue = true
    ...
    Popup {
        ...
        ColumnLayout {
            id: mainColumnLayout
            // width:  innerLayout.width + (_margins * 5)
            // height: innerLayout.height + (_margins * 8)
            ...
            ColumnLayout {
                ...
                QGCButton {
                    ...
                    onClicked: {
                        popup.close()
                        _customFunction.setRCUsername(rCText.text)
                        _customFunction.setGCSUsername(gCSText.text)
                        usernameCheck(_customFunction.getGCSUsername(), _customFunction.getRCUsername())
                    }
                }
            }
        }
    ...
    Popup {
        ...
        modal: true
        anchors.centerIn:   parent
        width:              mainColumnLayout2.width + (padding * 2)
        height:             mainColumnLayout2.y + mainColumnLayout2.height + padding
        padding:            2
        focus:              true
        closePolicy: Popup.NoAutoClose
        ...
        background: Item {
            ...
            Rectangle {
                anchors.margins:    usernameError.padding
                anchors.fill:       parent
                color:              _pal.window
            }
        }
        ...
        Rectangle {
            width:  mainColumnLayout2.width
            height: titleRowLayout2.height
            color:  qgcPal.windowShade
        }
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                QGCLabel {
                    Layout.leftMargin:  ScreenTools.defaultFontPixelWidth
                    Layout.fillWidth:   true
                    text:               gCSText.text === "" || rCText.text === "" ? 'Incomplete Username' : 'Incorrect Username'
                    font.pointSize:     ScreenTools.mediumFontPointSize
                    verticalAlignment:	Text.AlignVCenter
                }
...
//                spacing:        ScreenTools.defaultFontPixelWidth
                ...
                QGCLabel {
                    height:             _toolButtonHeight
                    Layout.fillWidth:   true
                    text: gCSText.text === "" || rCText.text === "" ? qsTr("Please enter usernames for both Mission Coordinator and Pilot.") : qsTr("Please enter the correct usernames.")
                    bottomPadding: 6
                }
            }
        }
    ...
    function usernameCheck(missionName, pilotName) { //new
        if (missionName === "" || pilotName === "") {
//            mainWindow.showMessageDialog("Incorrect Username", "Please enter the correct usernames.")
            usernameError.open()
        } else if (!_customFunction.verifyUsername(missionName) || !_customFunction.verifyUsername(pilotName)) {
            usernameError.open()
        } else {
            toolbar._gCSUsername = _customFunction.getGCSUsername()
            toolbar._rCUsername = _customFunction.getRCUsername()
        }
    }
    ...
    //new
    function showComponentDialogWithoutButton(component, title, charWidth) {
        var dialogWidth = charWidth === showDialogFullWidth ? mainWindow.width : ScreenTools.defaultFontPixelWidth * charWidth
        var dialog = dialogDrawerComponent.createObject(mainWindow, { width: dialogWidth, dialogComponent: component, dialogTitle: title })
        mainWindow.pushPreventViewSwitch()
        dialog.open()
    }
...
//            modal: true //new
//            closePolicy: Popup.NoAutoClose //new
        }
    }
    ...
    onClosing: {
        if (!_forceClose) {
            unsavedMissionCloseDialog.check() //comment when testing only
            close.accepted = false
        }
        _customFunction.removeSmartBattery()
//        _customFunction.removeTemp()
    }
            ...
            ColumnLayout {
                ...
                ColumnLayout {
                    ...
                    SubMenuButton {
                        ...
                        visible:            QGroundControl.corePlugin.showAdvancedUI //new
                        onClicked: {
                            if (!mainWindow.preventViewSwitch()) {
    ...
    function showTestingASelectDialog() {
        if (!mainWindow.preventViewSwitch()) {
            showPopupDialogFromComponent(testingADialogComponent)
        }
    }

    Component {
        id: testingADialogComponent

        QGCPopupDialog {
            id:         toolSelectDialog
            title:      qsTr("Pilot Username")
            buttons:    StandardButton.Close
//            visible: true
//            modal: true
            ...
            ColumnLayout {
                ...
                ColumnLayout {
                    ...
                    QGCButton {
                        ...
                        onClicked: {
                            toolSelectDialog.hideDialog()
//                            console.log(gCSText.text)
//                            console.log(rCText.text)
                            //check if fields are empty
                            //if not, change condition (global) so that mission can be started, (maybe after the slide to confirm dialog)
                            //if fields are empty, display message mission could not be started because pilot names are not filled in
                            //if not empty, save names and proceed to start mission
                        }
                    }
                }
            }
